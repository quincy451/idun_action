...
`FSign`, `FTrunc`, `FFloor`, `FMin`, and `FMax` for named REAL operands and a bounded,
...
Idun now lowers `FTrunc` and `FFloor` to the same independently selected target objects, but
...
- prune unreachable routines from full-source library includes, or generate
  ...
  `FFloor` are the first two bodies moved to independently selected shared OBJs
