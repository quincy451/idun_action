...
class TestIdunWorkspaceExport(unittest.TestCase):
    ...
    def test_export_contains_linux_tools_without_udos_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cos_object = (out / "LIB" / "RT_F_COS.OBJ").read_text(
                encoding="ascii"
            )
            self.assertIn("x rt_f_cos 0 609", cos_object)
            for dependency in (
                "rt_f_wrap_pi",
                "rt_f_cmp",
                "rt_f_sub",
            ...
            atan_object = (out / "LIB" / "RT_F_ATAN.OBJ").read_text(
                encoding="ascii"
            )
            self.assertIn("x rt_f_atan 0 1032", atan_object)
            for dependency in ("rt_f_div", "rt_f_sub", "rt_f_add", "rt_f_mul"):
                self.assertIn(f"u {dependency}", atan_object)
            self.assertNotIn("u rt_f_cmp", atan_object)
            self.assertIn("n rt_f_atan", atan_object)
            atan2_object = (out / "LIB" / "RT_F_ATAN2.OBJ").read_text(
                encoding="ascii"
            )
            self.assertIn("x rt_f_atan2 0 493", atan2_object)
            for dependency in ("rt_f_div", "rt_f_atan", "rt_f_add", "rt_f_sub"):
                self.assertIn(f"u {dependency}", atan2_object)
            self.assertNotIn("u rt_f_cmp", atan2_object)
            self.assertIn("n rt_f_atan2", atan2_object)
            asin_object = (out / "LIB" / "RT_F_ASIN.OBJ").read_text(
                encoding="ascii"
            )
            self.assertIn("x rt_f_asin 0 220", asin_object)
            for dependency in (
                "rt_f_mul",
                "rt_f_sub",
                "rt_f_sqrt",
            ...
            sec_object = (out / "LIB" / "RT_F_SEC.OBJ").read_text(
                encoding="ascii"
            )
            self.assertIn("x rt_f_sec 0 71", sec_object)
            for dependency in ("rt_f_cos", "rt_f_div"):
                self.assertIn(f"u {dependency}", sec_object)
            self.assertNotIn("u rt_f_cmp", sec_object)
            self.assertIn("n rt_f_sec", sec_object)
            csc_object = (out / "LIB" / "RT_F_CSC.OBJ").read_text(
                encoding="ascii"
            )
            self.assertIn("x rt_f_csc 0 71", csc_object)
            for dependency in ("rt_f_sin", "rt_f_div"):
                self.assertIn(f"u {dependency}", csc_object)
            self.assertNotIn("u rt_f_cmp", csc_object)
            self.assertIn("n rt_f_csc", csc_object)
            cot_object = (out / "LIB" / "RT_F_COT.OBJ").read_text(
                encoding="ascii"
            )
            self.assertIn("x rt_f_cot 0 113", cot_object)
            for dependency in ("rt_f_cos", "rt_f_sin", "rt_f_div"):
                self.assertIn(f"u {dependency}", cot_object)
            self.assertNotIn("u rt_f_cmp", cot_object)
            self.assertIn("n rt_f_cot", cot_object)
            asec_object = (out / "LIB" / "RT_F_ASEC.OBJ").read_text(
                encoding="ascii"
            )
            self.assertIn("x rt_f_asec 0 96", asec_object)
            for dependency in ("rt_f_div", "rt_f_acos"):
                self.assertIn(f"u {dependency}", asec_object)
            self.assertNotIn("u rt_f_cmp", asec_object)
            self.assertIn("n rt_f_asec", asec_object)
            acsc_object = (out / "LIB" / "RT_F_ACSC.OBJ").read_text(
                encoding="ascii"
            )
            self.assertIn("x rt_f_acsc 0 96", acsc_object)
            for dependency in ("rt_f_div", "rt_f_asin"):
            ...
            self.assertIn("x rt_f_exp 0 1465", exp_object)
            for dependency in (
                "rt_f_div",
                "rt_f_floor",
                "rt_f_to_i",
            ...
            for name in ("LOG2", "LOG10"):
                log_object = (out / "LIB" / f"RT_F_{name}.OBJ").read_text(
                    encoding="ascii"
                )
                self.assertIn(f"x rt_f_{name.lower()} 0 71", log_object)
                self.assertIn("u rt_f_ln", log_object)
                self.assertIn("u rt_f_div", log_object)
                self.assertIn(f"n rt_f_{name.lower()}", log_object)
            deg_to_rad_object = (
            ...
            self.assertIn("REAL FUNC FATan(REAL value)", math_header)
            self.assertNotIn("REAL magnitude,reduced,square,term,result", math_header)
