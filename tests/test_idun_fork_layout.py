...
class TestIdunForkLayout(unittest.TestCase):
    ...
    def test_cross_product_plan_tracks_native_real_progress_and_remaining_work(self) -> None:
        ...
        self.assertIn("inventories are 1,381 broad direct-PRG shapes", flat_parity)
        self.assertIn("196 non-runtime source-backed object-emission shapes", flat_parity)
        self.assertIn("315 compiled-runtime relocation-oracle cases", flat_parity)
        self.assertIn("thirty-two link-selected callable builtins", parity)
        self.assertIn("remaining 11 MATH1 routines", parity)
        ...
        self.assertIn("pass L is 6,125 bytes with 2,067 bytes free", flat_parity)
        self.assertIn("pass M is 6,993 bytes with 1,199 bytes free", flat_parity)
        self.assertIn("pass N is 7,115 bytes with 1,077 bytes free", flat_parity)
        self.assertIn("pass O is 7,118 bytes with 1,074 bytes free", flat_parity)
        self.assertIn("pass P is 7,142 bytes with 1,050 bytes free", flat_parity)
        self.assertIn("pass Q is 7,146 bytes with 1,046 bytes free", flat_parity)
        self.assertIn("pass R is 7,329 bytes with 863 bytes free", flat_parity)
        self.assertIn("pass S is 7,823 bytes with 369 bytes free", flat_parity)
        self.assertIn("pass T is 8,142 bytes with 50 bytes free", flat_parity)
        self.assertIn("Pass U is 7,470 bytes with 722 bytes free", flat_parity)
        ...
        self.assertIn("Current native inventories are 1,381 broad", flat_handoff)
        self.assertIn("196 non-runtime source-backed", flat_handoff)
        self.assertIn("315 compiled-runtime relocation-oracle cases", flat_handoff)
        ...
        self.assertIn("Pass S is 7,823 bytes with 369 bytes free", flat_handoff)
        ...
        self.assertIn("Pass T is 8,142 bytes with 50 bytes free", flat_handoff)
        self.assertIn(
            "pass 6 is 8,095 bytes with 97 bytes free",
            " ".join(handoff.split()),
        )
        self.assertIn(
            "pass 7 is 7,195 bytes with 997 bytes free",
            " ".join(handoff.split()),
        )
        self.assertIn(
            "native MATH1 gap is now 11 public routines",
            " ".join(handoff.split()),
        )
