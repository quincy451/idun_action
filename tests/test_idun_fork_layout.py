...
class TestIdunForkLayout(unittest.TestCase):
    ...
    def test_cross_product_plan_tracks_native_real_progress_and_remaining_work(self) -> None:
        ...
        self.assertIn("inventories are 1,371 broad direct-PRG shapes", flat_parity)
        self.assertIn("196 non-runtime source-backed object-emission shapes", flat_parity)
        self.assertIn("305 compiled-runtime relocation-oracle cases", flat_parity)
        ...
        self.assertIn("pass P is 7,109 bytes with 1,083 bytes free", flat_parity)
        self.assertIn("pass Q is 7,113 bytes with 1,079 bytes free", flat_parity)
        ...
        self.assertIn("Current native inventories are 1,371 broad", flat_handoff)
        self.assertIn("196 non-runtime source-backed", flat_handoff)
        self.assertIn("305 compiled-runtime relocation-oracle cases", flat_handoff)
        ...
        self.assertIn("Pass S is 7,790 bytes with 402 bytes free", flat_handoff)
        ...
        self.assertIn("Pass T is 8,109 bytes with 83 bytes free", flat_handoff)
        self.assertIn(
            "pass 6 is 8,089 bytes with 103 bytes free",
            " ".join(handoff.split()),
        )
        self.assertIn(
            "pass 7 is 6,870 bytes with 1,322 bytes free",
            " ".join(handoff.split()),
        )
        self.assertIn(
            "native MATH1 gap is now 21 public routines",
            " ".join(handoff.split()),
        )
