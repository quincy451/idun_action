...
| `VM` `AVM1` runner | Yes | Yes | Partial | Yes | `AVMRUN.PRG` now runs a flagged Acheron-backed `AVM1` subset from the mounted Action workspace. Current proof surface is intentionally small but real: `setp16`, `calln`, `native`, `stringz`, `jump`, `call`, `ret`, and a narrow interpreted stack subset for `push16`, `add`, `sub`, `eq`, `ne`, `lt`, `gt`, `band`, `bor`, `bxor`, `shl1`, and `shr1`. The focused runtime proofs now include executing the live `ALINK` output through `AVMRUN.PRG` plus compiler-lowered runtime comparison cases such as `<=` / `>=`. |
...
| Link `.AVO` -> `AVM1` with dead-strip | Yes | No | Partial | Yes | `ALINK.PRG` now loads deterministic `AVO1` object stubs from `OBJ/`, parses export/body/payload metadata plus unresolved external symbol lines, uses compiler-emitted export sizes to recover procedure boundaries, resolves the current widened unresolved external closure by loading secondary objects, carries child-object integer and string literal pools into the linked image, and writes `BIN/<NAME>.AVM` on the host fs tree as direct binary `AVM1`. The focused proof verifies the exact emitted bytes and proves the linked artifact executes through `AVMRUN.PRG`. This is still a narrow linker slice, not a full historical dead-strip object merger. |
