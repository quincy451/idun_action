...
|---|---:|---:|---:|---:|---|
| `ACTC` compiler | Yes | No | Partial | Yes | `ACTC.PRG` is now the first UDOS-native compiler front-end slice. The current proof validates a tracked project module, verifies the source `MODULE` header matches the requested module name, extracts top-level `PROC` exports from source, scans source text for the current runtime-call marker set, folds the current narrow decimal `PrintI` / `PrintIE` `+` / `-` expressions before object emission, and emits a deterministic `OBJ/<NAME>.AVO` object stub with module/import metadata, export offset/size triplets, compiler-emitted `body_ops`, a minimal local control-flow `payload_hex` skeleton (`CALL` local proc, `RET`), and explicit `payload_bytes`. The older flat `calls` list is no longer emitted because local body semantics now live in `body_ops`. It is not full code generation yet, and the focused runtime proof verifies the host-side object file rather than shell-side `TYPE OBJ/...` readback. |
| `ALINK` dead-strip linker | Yes | No | Partial | Yes | `ALINK.PRG` is now the first UDOS-native linker slice that emits a final-image artifact instead of only a planning report. The current proof loads deterministic `OBJ/*.AVO` text object stubs, parses export/body/payload metadata plus unresolved external symbol lines, uses compiler-owned export offset/size triplets plus `body_ops` instead of reconstructing procedure bodies from a flat call list, seeds the local live set from the module entry proc instead of assuming export slot `0`, propagates the local body-op call graph, resolves the current widened small-object closure, and emits `BIN/<NAME>.AVMTXT` on the host fs tree as compact AVM byte text (`entry 0`, `code $..`, `hex ...`). The focused proof host-packs that text through `avm_pack.py --text --flags 1`, verifies the exact `AVM1` bytes, and proves an unused local export is stripped from the final image. The direct typed `make vice-action-alink-avmrun` path now also proves the emitted artifact executes through `AVMRUN.PRG` and prints the expected `HELLOTOOL7` / `42` output before returning to the UDOS prompt. The current `ALINK.PRG` footprint is `4137` bytes. It is not a full object merger or direct on-target binary emitter yet. |
...
| `VM` `AVM1` runner | Yes | Yes | Partial | Yes | `AVMRUN.PRG` now runs a flagged Acheron-backed `AVM1` subset from the mounted Action workspace. Current proof surface is intentionally small but real: `setp16`, `calln`, `native`, `stringz`, `jump`, `call`, `ret`, and the compact `entry/code/hex` text contract that `ALINK.PRG` now emits. The focused runtime proof now includes executing the live `ALINK` output through `AVMRUN.PRG`. |
...
| Emit `.ACT` -> `.AVO` object stubs | Yes | No | Partial | Yes | `ACTC.PRG` now emits deterministic `AVO1` text object stubs into `OBJ/` for tracked project modules, verifies the source `MODULE` header matches the requested module name, extracts top-level `PROC` exports with real local offset/size triplets, emits compiler-owned `body_ops`, folds the current narrow decimal `PrintI` / `PrintIE` `+` / `-` expressions before object emission, emits a minimal local control-flow `payload_hex` skeleton plus `payload_bytes`, and includes the current inferred runtime-import list from simple source-pattern scanning. The current UDOS-native slice is a project-aware front end/object emitter only, not a full compiler. |
| Emit `.AVO` -> `.AVMTXT` final-image skeleton | Yes | No | Partial | Yes | `ALINK.PRG` now loads deterministic `AVO1` object stubs from `OBJ/`, parses export/body/payload metadata plus unresolved external symbol lines, uses compiler-emitted export sizes to recover procedure boundaries, and writes `BIN/<NAME>.AVMTXT` on the host fs tree as compact AVM byte text with `entry 0`, `code $..`, and `hex ...`. The current focused proof resolves a widened unresolved external closure by loading secondary objects, emitting their live bytes into the final image, and carrying child-object integer and string literal pools through to the linked text. The focused proof then host-packs that text through `avm_pack.py --text --flags 1`, verifies the exact `AVM1` bytes, and proves the emitted artifact executes through `AVMRUN.PRG`. The current `ALINK.PRG` footprint is `4137` bytes. This is closer to the final linker artifact, but it is still not direct on-target binary emission or a full dead-strip object merger. |
