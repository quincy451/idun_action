...
|---|---:|---:|---:|---:|---|
| `ACTC` compiler | Yes | No | Partial | Yes | `ACTC.PRG` is now the first UDOS-native compiler front-end slice. The current proof validates a tracked project module, verifies the source `MODULE` header matches the requested module name, extracts top-level `PROC` exports from source, extracts flat local inter-proc calls, scans source text for the current runtime-call marker set, and emits a deterministic `OBJ/<NAME>.AVO` object stub with module/export/call/import/payload metadata. It is not full code generation yet, and the focused runtime proof verifies the host-side object file rather than shell-side `TYPE OBJ/...` readback. |
