...
|---|---:|---:|---:|---:|---|
| `ACTC` compiler | Yes | No | Partial | Yes | `ACTC.PRG` is now the first UDOS-native compiler front-end slice. The current proof validates a tracked project module, verifies the source `MODULE` header matches the requested module name, extracts top-level `PROC` exports from source, extracts pair-form local inter-proc calls, scans source text for the current runtime-call marker set, and emits a deterministic `OBJ/<NAME>.AVO` object stub with module/export/call/import metadata plus a minimal local control-flow `payload_hex` skeleton (`CALL` local proc, `RET`) and explicit `payload_bytes`. It is not full code generation yet, and the focused runtime proof verifies the host-side object file rather than shell-side `TYPE OBJ/...` readback. |
| `ALINK` dead-strip linker | Yes | No | Partial | Yes | `ALINK.PRG` is now the first UDOS-native linker slice that emits a final-image artifact instead of only a planning report. The current proof loads a deterministic `OBJ/<NAME>.AVO` text object stub, parses its export/call/import/payload metadata, seeds the local live set from the module entry proc instead of assuming export slot `0`, propagates the pair-call graph, resolves the current small runtime closure, and emits `BIN/<NAME>.AVM.TXT` on the host fs tree as symbolic AVM text (`entry main`, labels, `call`, `ret`) assembled from linker metadata instead of copied `payload_hex`. The focused proof host-packs that text through `avm_pack.py --text --flags 1`, verifies the exact `AVM1` bytes, and proves an unused local export is stripped from the final image. It is not a full object merger or direct on-target binary emitter yet. |
