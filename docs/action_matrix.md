...
| `ALINK` dead-strip linker | Yes | No | Partial | Yes | `ALINK.PRG` is now the first UDOS-native linker slice that emits a final-image artifact instead of only a planning report. The current proof loads deterministic `OBJ/*.AVO` text object stubs, parses export/body/import/payload metadata plus unresolved external symbol lines, uses compiler-owned export offset/size triplets plus `body_ops` instead of reconstructing procedure bodies from a flat call list, seeds the local live set from the module entry proc instead of assuming export slot `0`, propagates the local body-op call graph, resolves the current small runtime closure, and emits `BIN/<NAME>.AVMTXT` on the host fs tree as symbolic AVM text (`entry main`, labels, `call`, `ret`). The current focused proof now loads a small external-object closure for unresolved symbols, emits that live code before literal data, and carries child-object integer and string literal pools into the symbolic image. A dead-code cleanup dropped the current `ALINK.PRG` footprint to `3604` bytes while keeping that proof green. The focused proof host-packs that text through `avm_pack.py --text --flags 1`, verifies the exact `AVM1` bytes, and proves an unused local export is stripped from the final image. It is not a full object merger or direct on-target binary emitter yet. |
...
| Emit `.AVO` -> `.AVMTXT` final-image skeleton | Yes | No | Partial | Yes | `ALINK.PRG` now loads deterministic `AVO1` object stubs from `OBJ/`, parses export/body/import/payload metadata plus unresolved external symbol lines, uses compiler-emitted export sizes to recover procedure boundaries, and writes `BIN/<NAME>.AVMTXT` on the host fs tree as symbolic AVM text with an entry label plus emitted labels, `call`, and `ret` directives reconstructed from `body_ops`. The current focused proof resolves a small unresolved external closure by loading secondary objects, emitting their live code into the final symbolic image, and carrying child-object integer and string literal pools through to the linked text. A dead-code cleanup dropped the current `ALINK.PRG` footprint to `3604` bytes while keeping the focused proof green. The focused proof then host-packs that text through `avm_pack.py --text --flags 1`, verifies the exact `AVM1` bytes, and proves an unused local export is dead-stripped from the final image. This is closer to the final linker artifact, but it is still not direct on-target binary emission or a full dead-strip object merger. |
