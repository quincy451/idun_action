...
| `VM` `AVM1` runner | Yes | Yes | Partial | Yes | `AVMRUN.PRG` now runs a flagged Acheron-backed `AVM1` subset from the mounted Action workspace. Current proof surface is intentionally small but real: `setp16`, `calln`, `native`, `stringz`, `jump`, `call`, `ret`, and a narrow interpreted stack subset for `push16`, `add`, `sub`, `eq`, `ne`, `lt`, and `gt`. The focused runtime proofs now include executing the live `ALINK` output through `AVMRUN.PRG` plus compiler-lowered runtime comparison cases such as `<=` / `>=`. |
