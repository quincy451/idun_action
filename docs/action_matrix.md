...
| `ALINK` dead-strip linker | Yes | No | Partial | Yes | `ALINK.PRG` is now the first UDOS-native linker slice. The current proof loads a deterministic `OBJ/<NAME>.AVO` text object stub, parses its export/call/import metadata, resolves the current small runtime closure, and emits `BIN/<NAME>.MAP` on the host fs tree with export, call, include, and resolve lines. It is not a full object merger or final AVM linker yet. |
...
| Emit `.AVO` -> `.MAP` dependency maps | Yes | No | Partial | Yes | `ALINK.PRG` now loads deterministic `AVO1` object stubs from `OBJ/`, parses export/call/import metadata, resolves the current small runtime import closure, and writes `BIN/<NAME>.MAP` on the host fs tree with export, call, include, and resolve lines. The current UDOS-native slice is a map/link planning proof, not a full dead-strip object merger or final AVM image linker. |
