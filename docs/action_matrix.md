...
| `ALINK` dead-strip linker | Yes | No | Partial | Yes | `ALINK.PRG` is now the first UDOS-native linker slice that emits a final-image artifact instead of only a planning report. The current proof loads a deterministic `OBJ/<NAME>.AVO` text object stub, parses export/body/import/payload metadata plus unresolved external symbol lines, uses compiler-owned export offset/size triplets plus `body_ops` instead of reconstructing procedure bodies from a flat call list, seeds the local live set from the module entry proc instead of assuming export slot `0`, propagates the local body-op call graph, resolves the current small runtime closure, and emits `BIN/<NAME>.AVMTXT` on the host fs tree as symbolic AVM text (`entry main`, labels, `call`, `ret`). The current focused proof also loads one secondary object for an unresolved external symbol and emits its live code into the symbolic image before literal data. The focused proof host-packs that text through `avm_pack.py --text --flags 1`, verifies the exact `AVM1` bytes, and proves an unused local export is stripped from the final image. It is not a full object merger or direct on-target binary emitter yet. |
