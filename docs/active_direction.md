...
3. Keep shared Action source, OBJ1 records, link-selected 6502 runtime behavior,
   ...
   Native named-REAL `FSign`, `FTrunc`, `FFloor`, `FCeil`, `FMin`, and `FMax` calls plus its bounded
   ...
   Linux ACTC now lowers `FTrunc`, `FFloor`, and `FCeil` to the same shared objects
