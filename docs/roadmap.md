...
- Exact host checks, 116 Idun VICE vectors, and a focused native direct PRG
  ...
  pruning.
- At that checkpoint native inventories were 1,338 broad and 295
  compiled-runtime cases; native pass 6 was 8,074 bytes with 118 bytes free.
  The native MATH1 gap was 31 public routines, and the remaining Idun source
  bodies still needed reachable-only packaging.

## 2026-07-20 Shared MATH1 Fractional Part

- Added the 93-byte `RT_F_FRAC.OBJ` to the shared manifest. It imports
  `RT_F_TRUNC.OBJ` and `RT_F_SUB.OBJ`, computes `value-FTrunc(value)`, and is
  safe for aliased source/destination pointers.
- Linux ACTC parses and constant-folds `FFrac`, emits `RT_F_FRAC` only for
  dynamic expressions, and no longer compiles a portable MATH1 fractional-part
  body. Native ACTC recognizes assignment, direct-print, and REAL-condition
  forms.
- Exact host checks, 116 Idun VICE vectors, and a focused native direct PRG
...
## 2026-07-22 Native Named CARD FOR-Bound Parity

- Native ActionC64U added pass T, `ACTC_OVLT.BIN` (id 29), for named CARD
  initial/final bounds in the bounded pass-S REAL-function `FOR` form. Bounds
  are staged once per loop entry; steps remain nonzero signed constants.
- Added byte-identical `real_function_dynamic_for_postfix.act` fixtures. Native
  ACTC/ALINK/VICE and this fork's ACTC/ALINK generated-6502 path both execute
  nested `FOR J=I TO 3` and `FOR L=1 TO K` forms, produce 7.0 twice, and the
  native direct PRG displays `77`.
...
## 2026-07-23 Shared MATH1 Cosine

- Synchronized the independent 609-byte `RT_F_COS.OBJ`, math generator, and
  shared manifest from native ActionC64U.
- Linux ACTC recognizes `FCos(value)` as a unary REAL intrinsic and imports
  `RT_F_COS` rather than compiling the portable source body. Constant calls
  deliberately use the target helper rather than host folding.
- The helper shares the private wrap-to-pi dependency, folds to the central
  half-pi interval, and evaluates the portable degree-10 even polynomial with
  binary32 rounding after every operation. It preserves aliased source and
  destination pointers and does not import the FSin root.
- The focused native ACTC/ALINK/VICE PRG prints `-0.416146...` for `FCos(2)`.
  Idun's generated MATH1 PRG executes both FSin and FCos through the shared
  target objects.
- Current native inventories are 1,373 broad direct-PRG, 196 non-runtime
  source-backed, and 307 compiled-runtime relocation-oracle cases. The native
  suite remains 866 tests, including 252 overlay and 199 source-cache tests;
  Idun retains its 154-test host, 139-test sanitizer, and 21-test direct-PRG
  gates.
- Native pass 6 is 8,085 bytes with 107 bytes free; pass 7 is 6,932 bytes with
  1,260 bytes free; passes L through U are respectively 6,105, 6,974, 7,096,
  7,099, 7,123, 7,127, 7,310, 7,804, 8,123, and 7,453 bytes.
...
- Synchronized the independent 493-byte `RT_F_ATAN2.OBJ`, math generator, and
  shared manifest from native ActionC64U.
- Linux ACTC recognizes `FATan2(y,x)` as a binary REAL intrinsic and imports
  `RT_F_ATAN2` rather than compiling the portable source body. The native
  compiler selects the same root only when referenced.
- The alias-safe helper implements canonical-NaN, signed-zero, zero-axis,
  infinity, and all four quadrant rules. Ordinary finite operands use
  `RT_F_DIV`, `RT_F_ATAN`, and an add/subtract pi correction.
- The exact 6502 verifier covers edge/random inputs and both input-alias forms.
  ...
  `FATan2(-1,-1)`.
- Current native inventories are 1,376 broad direct-PRG, 196 non-runtime
  source-backed, and 310 compiled-runtime relocation-oracle cases. The native
  suite remains 866 tests, including 252 overlay and 199 source-cache tests;
  Idun retains its 154-test host, 139-test sanitizer, and 21-test direct-PRG
  gates.
- Native pass 6 is 8,061 bytes with 131 bytes free; pass 7 is 7,031 bytes with
  1,161 bytes free; pass K is 5,899 bytes with 2,293 bytes free; passes L
  through U are respectively 6,129, 6,998, 7,120, 7,123, 7,147, 7,151, 7,334,
  7,828, 8,147, and 7,477 bytes.
...
- Synchronized the independent 220-byte `RT_F_ASIN.OBJ`, math generator, and
  shared manifest from native ActionC64U.
- Linux ACTC recognizes `FASin(value)` as a unary REAL intrinsic and imports
  `RT_F_ASIN` rather than compiling the portable source body. The native
  compiler selects the same root only when referenced.
- The alias-safe helper preserves signed zero, returns signed binary32 pi/2 at
  the endpoints, canonicalizes NaN or magnitudes above one, and evaluates
  ordinary inputs through multiplication, subtraction, square root, and
...
- Current native inventories are 1,377 broad direct-PRG, 196 non-runtime
  source-backed, and 311 compiled-runtime relocation-oracle cases.
- Native pass 6 is 8,069 bytes with 123 bytes free; pass 7 is 7,065 bytes with
  1,127 bytes free; pass K is 5,899 bytes with 2,293 bytes free; passes L
  through U are respectively 6,137, 7,006, 7,128, 7,131, 7,155, 7,159, 7,342,
...
## 2026-07-24 Shared MATH1 Inverse Secant

- Synchronized the independent 96-byte `RT_F_ASEC.OBJ`, math generator, and
  shared manifest from native ActionC64U.
- Linux ACTC recognizes `FASec(value)` as a unary REAL intrinsic and imports
  `RT_F_ASEC` rather than compiling the portable source body. Native ACTC
  selects the same root only when referenced.
- The alias-safe helper evaluates source-defined binary32
  `FACos(1.0/value)` in source order. It directly imports only division and
  inverse cosine, yielding a 4,723-byte linked closure without comparison,
  tangent, secant, cosecant, cotangent, or power wrapper roots.
- Exact 6502 verification covers 2,087 edge/random inputs and every in-place
  alias case. The focused native ACTC/ALINK/VICE PRG prints `1.047197...` for
  `FASec(2)`, and Idun's generated MATH1 PRG executes the same call.
- Validation passes the complete 1,382-shape native direct-PRG matrix, all 69
  launches in the live native math-runtime target, 133 UDOS tests with one
  documented skip, 866 ActionC64U tests, and Idun's 154 host, 139 sanitizer,
  and 21 direct-PRG tests with one documented VICE skip.
- Current native inventories are 1,382 broad direct-PRG, 196 non-runtime
  source-backed, and 316 compiled-runtime relocation-oracle cases.
- Native pass 6 is 8,094 bytes with 98 bytes free; pass 7 is 7,229 bytes with
  963 bytes free; pass K is 5,899 bytes with 2,293 bytes free; passes L
  through U are respectively 6,132, 7,000, 7,122, 7,125, 7,149, 7,153, 7,336,
  7,830, 8,149, and 7,477 bytes.
- Native MATH1 now exposes thirty-three link-selected calls and lacks 10 public
  routines. FACsc is the next dependency-ordered public routine.

## 2026-07-24 Shared MATH1 Inverse Cosecant

- Synchronized the independent 96-byte `RT_F_ACSC.OBJ`, math generator, and
  shared manifest from native ActionC64U.
- Linux ACTC recognizes `FACsc(value)` as a unary REAL intrinsic and imports
  `RT_F_ACSC` rather than compiling the portable source body. Native ACTC
  selects the same root only when referenced.
- The alias-safe helper evaluates source-defined binary32
  `FASin(1.0/value)` in source order. It directly imports only division and
  inverse sine, yielding a 4,652-byte linked closure without inverse cosine,
  comparison, tangent, secant, cosecant, cotangent, or power wrapper roots.
- Exact 6502 verification covers 2,087 edge/random inputs and every in-place
  alias case. The focused native ACTC/ALINK/VICE PRG prints `0.523598...` for
  `FACsc(2)`, and Idun's generated 22-result MATH1 PRG executes the same call.
- Validation passes the complete 1,383-shape native direct-PRG matrix, all 70
...
- Current native inventories are 1,383 broad direct-PRG, 196 non-runtime
  source-backed, and 317 compiled-runtime relocation-oracle cases.
- Native pass 6 is 8,096 bytes with exactly 96 bytes free; pass 7 is 7,260
  bytes with 932 bytes free; pass K is 5,899 bytes with 2,293 bytes free;
  passes L through U are respectively 6,139, 7,007, 7,129, 7,132, 7,156,
