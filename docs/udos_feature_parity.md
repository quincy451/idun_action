...
executes the same call.

The inverse-cosecant slice adds the independent 96-byte `RT_F_ACSC.OBJ` root.
Both compilers lower `FACsc(value)` directly to this alias-safe unary object.
It snapshots the source, evaluates binary32 `FASin(1.0/value)` in portable
operation order, and directly imports only division and inverse sine. ALINK
